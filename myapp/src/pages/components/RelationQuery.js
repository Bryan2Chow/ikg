...
export default class RelationQuery extends Component {
    ...
    SearchBasedRelation = () => {
        ...
        const selectedFact = this.state.data.links.filter((linkObj) => {
            console.log(this.state.Fact.Tail)
            return linkObj.source === this.state.Fact.Head && linkObj.target === this.state.Fact.Tail && linkObj.label === this.state.Fact.Relation
        })
        const newNodess = this.state.data.nodes.filter((nodesobj) => {
            return selectedFact.find((nelinksobj) => {
                ...
                <div style={{ marginTop: 50 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Select
                            ...
                            onChange={this.RelationType}
                        >
                            {this.state.Relationship.map((Relationship) => (
                                <Option key={Relationship}>{Relationship}</Option>
                            ))}
                        </Select>
                        <Input placeholder='输入尾实体' style={{ width: "20%" }} onChange={this.TailEntity} />
